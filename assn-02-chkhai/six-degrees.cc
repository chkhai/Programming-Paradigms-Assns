...
void generateShortestPath(string& source, string& target, imdb& db){
  ...
  while(!all_paths.empty() && all_paths.front().getLength() < 6){
    path curr_path = all_paths.front();
    all_paths.pop_front();
    vector<film> films;
    string curr_actor = curr_path.getLastPlayer();
    db.getCredits(curr_actor, films);

    for(const film& f : films){
      if(usedFilms.find(f) != usedFilms.end()) continue;
      usedFilms.insert(f);
      vector<string> cast;
      db.getCast(f, cast);
      for(const string& actor : cast){
        if(usedActors.find(actor) != usedActors.end()) continue;
