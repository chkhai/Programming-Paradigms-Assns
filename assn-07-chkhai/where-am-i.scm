...
(define (intersection-points lst)
  (if (= (length lst) 1) '()
    (append (intersect (car lst) (cadr lst)) ;; intersection of first and second
            (intersection-points (cons (car lst) (cddr lst))) ;; intersection of cons of all lists except second
            (intersection-points (cdr lst)) ;; intersection of all lists except first
    )
  )
)
...
(define (rate-points lst)
  (map 
    (lambda (curr) (list (distance-product curr lst) curr)) ;;fn
    lst ;;lst
  )
)
...
(define (sort-points lst)
  (quicksort 
    lst ;; lst
    (lambda (pt1 pt2) (< (car pt1) (car pt2))) ;; cmp
  )
)
...
(define (clumped-points lst)
  (map
    cadr ;; fn
    (prefix-of-list ;;lst
      (sort-points (rate-points lst))   ;;ls
      (quotient (length lst) 2)         ;;k
    )
  )
)
...
(define (average-point lst)
  (let ((dummy 
            (list (/ (apply + (map car lst)) (length lst))
                  (/ (apply + (map cadr lst)) (length lst))
            )
          )
        )
    (list (distance-product dummy lst) dummy)
  )
)
...
(define (where-am-i distances stars)
  (prefix-of-list 
    (sort-points 
      (map 
        best-estimate ;;fn
        (all-guesses distances stars) ;;lst
      )
    ) 
  6
  )
