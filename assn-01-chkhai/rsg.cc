...
string generateSentences(string& str, map<string, Definition>& grammar){
  if(grammar.find(str) == grammar.end()){
    return str;
  }
  Definition currDef = grammar[str];
  Production p = currDef.getRandomProduction();
  string r;
  for(string& s : p){
    r += generateSentences(s, grammar) + " ";
  }
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 3; i++){
    string s = "<start>";
    string res = generateSentences(s, grammar);
